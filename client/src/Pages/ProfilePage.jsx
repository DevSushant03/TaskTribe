...
const demoUser = {
  ...
  skills: ['React', 'Node.js', 'MongoDB', 'JavaScript', 'TypeScript', 'Tailwind CSS', 'Express.js', 'Git'],
  role: 'user',
  rating: {
    avg: 4.7,
    count: 23
    ...
    return (
      <div className="flex items-center gap-1">
        {[...Array(fullStars)].map((_, i) => (
          <FaStar key={i} className="text-orange-500 fill-orange-500" size={16} />
        ))}
        {hasHalfStar && <FaStar className="text-orange-500 fill-orange-500/50" size={16} />}
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-[#0C0C0C] min-h-screen">
        <div className="text-center">
          <FaSpinner className="animate-spin text-orange-600 mx-auto mb-4" size={40} />
            ...
            {user.rating?.avg > 0 ? (
              <div className="space-y-2">
                ...
                <p className="text-gray-500 text-sm">
                  Based on {user.rating?.count || 0} {user.rating?.count === 1 ? 'review' : 'reviews'}
                </p>
              </div>
            ) : (
